...
const DoctorCard = ({
  ...
  const starRating = ratings ? "⭐".repeat(ratings) : "";
    ...
    const fetchAppointments = async () => {
      ...
      try {
        ...
        const doctorAppointments = data.filter(
          (a: Appointment) =>
            a.doctorName === name && a.doctorSpeciality === speciality
        );
  ...
  const handleFormSubmit = async (appointmentData: AppointmentFormData) => {
          ...
          {...({
            trigger: (
              ...
              >
                ...
                {isExpired && "Book Again"}
                {!isBooked && !isExpired && !isPending && "Book Appointment"}
